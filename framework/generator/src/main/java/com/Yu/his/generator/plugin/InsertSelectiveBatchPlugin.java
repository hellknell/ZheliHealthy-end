...
//                .findFirst()
//                .orElse(null);
        IntrospectedColumn primaryKeyColumn = introspectedTable.getAllColumns().stream().filter(p -> p.getActualColumnName().equals("id") || p.getActualColumnName().equals("file_id")).findFirst().orElse(null);
